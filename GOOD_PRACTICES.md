...
- [ ] **Top-level error handling with exit codes (Critical for CI/CD)**
  ```python
  import sys
  ...
  def main() -> None:
      ...
      try:
          ...
          sys.exit(1)  # Failure (CI/CD detects this)
  ```
  - Exit code 0 = success (allows CI/CD to proceed)
  - Exit code 1 = fatal error (CI/CD fails the build)
  - Used by GitHub Actions, GitLab CI, Jenkins, etc. for failure detection

- [ ] **Log elapsed time for performance observability**
  ```python
...
- [ ] **Monitor in production**
  ...
  - Log entry/exit of critical functions with timing data
